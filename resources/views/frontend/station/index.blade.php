        ...
        <div class="container-fluid">
            <div class="border-bottom pt-6">
                <div class="row align-items-center">
                    <div class="col-sm col-12"><h1 class="h2 ls-tight">Trạm</h1></div>
                    <div class="col-sm-auto col-12 mt-4 mt-sm-0">
                        <div class="hstack gap-2 justify-content-sm-end">
                            ...
                            <a href="{{route('station.create')}}" class="btn btn-sm btn-primary">
                                ...
                                <span>Tạo trạm cho khách hàng</span>
                            </a>
                        </div>
                    </div>
                </div>
                <ul class="nav nav-tabs overflow-x border-0">
                    <li class="nav-item"><a href="#" class="nav-link active">Tất cả trạm</a></li>
                    <li class="nav-item">
            ...
            <div class="vstack gap-4">
                <div class="d-flex justify-content-between flex-column flex-sm-row gap-3">
                    <div class="hstack gap-2">
                        <div class="input-group input-group-sm input-group-inline"><span
                                class="input-group-text pe-2"><i class="bi bi-search"></i> 
                            </span>
                ...
                <div class="card">
                    <div class="card-header border-bottom d-flex align-items-center">
                        <h5 class="me-auto">Tất cả trạm</h5>
                    </div>
                </div>
                <div class="table-responsive">
                    <table class="table table-hover table-nowrap">
                        ...
                        <tr>
                            ...
                            <th  scope="col">Hình ảnh trạm</th>
                            <th  scope="col">Địa chỉ</th>
                            <th  scope="col">Uid</th>
                            <th  scope="col">Trạm thuộc tài khoản</th>
                            <th  scope="col">Trạng thái Trạm</th>
                            <th></th>
                        </tr>
                        </thead>
                        <tbody>
                        @foreach($station as $val)
                            <tr>
                                ...
                                <td><img src="{{asset('uploads/'.$val->image)}}" alt=""></td>
                                <td>{{$val->address}}</td>
                                <td>{{$val->uid}}</td>
                                <td><span class="badge text-uppercase bg-soft-primary text-primary rounded-pill">{{$val->customers->name}}</span></td>
                                <td>
                                    @if($val->status == 'on')
                                ...
                                <td class="text-end">
                                    <a href="{{route('station.edit', $val->id)}}" class="btn btn-sm btn-square btn-neutral">
                                        <i class="bi bi-pencil"></i>
                                    </a>
                                    <form action="{{route('station.destroy', $val->id)}}" method="POST">
                                        @csrf
                                        {{ method_field('DELETE') }}
                ...
                <div >
                    {{ $station->links('frontend.layout.pagination') }}
                </div>
